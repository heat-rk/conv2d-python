...
class Conv2D:
    ...
    def forward(self, input, weight, bias):
        ...
        for b in range(batches):
            h_in = input[b].shape[1]
            w_in = input[b].shape[2]

            if self.kernel_size[0] > h_in or self.kernel_size[1] > w_in:
                raise ValueError('kernel size can\'t be greater than input size')

            h_out = int(
                (h_in + 2 * self.pad[0] - self.dilation[0] * (self.kernel_size[0] - 1) - 1) / (self.stride[0]) + 1)

            w_out = int(
                (w_in + 2 * self.pad[1] - self.dilation[1] * (self.kernel_size[1] - 1) - 1) / (self.stride[1]) + 1)
                        ...
                        for c_in in range(self.in_channels):
                            for kernel_y in range(self.kernel_size[0]):
                                for kernel_x in range(self.kernel_size[1]):
                                    y_in = y_out * self.stride[0] + kernel_y * self.dilation[0] - self.pad[0]
                                    x_in = x_out * self.stride[1] + kernel_x * self.dilation[1] - self.pad[1]
